...
int main (int argc, char **argv)
{
  ...
  std::vector<std::string> keys_0(rx_info_0.keys());
  std::vector<std::string> vals_0(rx_info_0.vals());
  std::cout << "Channel 0\n----------\n";
  for(size_t n = 0; n < rx_info_0.size(); n++)
  {
    if (n == 3)
      std::cout << keys_0[n] << "\t\t: " << vals_0[n] << std::endl;
    else
      std::cout << keys_0[n] << "\t: " << vals_0[n] << std::endl;
  }
  std::cout << "----------\n";
  uhd::dict<std::string, std::string> rx_info_1 = rx->get_usrp_rx_info(1);
  std::vector<std::string> keys_1(rx_info_1.keys());
  std::vector<std::string> vals_1(rx_info_1.vals());
  std::cout << "Channel 1\n----------\n";
  for(size_t n = 0; n < rx_info_1.size(); n++)
  {
    if (n == 3)
      std::cout << keys_1[n] << "\t\t: " << vals_1[n] << std::endl;
    else
      std::cout << keys_1[n] << "\t: " << vals_1[n] << std::endl;
  }
  std::cout << "----------\n";
